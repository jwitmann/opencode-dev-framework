...
Because OpenCode does not expose a blocking completion hook, the gate is **advisory**. It cannot force the agent to keep working. Make the failure message extremely visible.

## Custom command

`/df-verify` is a custom OpenCode command that runs the completion gate on demand. It is defined in `commands/df-verify.md` (markdown command) or generated by the plugin into the command registry.
